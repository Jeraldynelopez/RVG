...
def fases_del_suelo_vol(volumen_solidos,volumen_agua,volumen_aire):
  ...
  plt.fill_between(np.arange(0, 100), 0, volumen_solidos[0], color='brown', alpha=0.5, label='Sólidos')
  plt.fill_between(np.arange(0, 100), v_solidos[0], v_solidos[0]+v_agua[0], color='blue', alpha=0.5, label='Agua')
  plt.fill_between(np.arange(0, 100), v_solidos[0]+v_agua[0], v_solidos[0]+v_agua[0]+v_aire[0], color='gray', alpha=0.5, label='Aire')
  plt.ylim(0,v_limites)
  plt.legend()
  plt.show()
...
def fases_del_suelo_grav(peso_solidos,peso_agua,peso_aire):
  ...
  plt.fill_between(np.arange(0, 100), 0, p_solidos[0], color='brown', alpha=0.5, label='Sólidos')
  plt.fill_between(np.arange(0, 100), p_solidos[0], p_solidos[0]+p_agua[0], color='blue', alpha=0.5, label='Agua')
  plt.fill_between(np.arange(0, 100), p_solidos[0]+p_agua[0], p_solidos[0]+p_agua[0]+p_aire[0], color='gray', alpha=0.5, label='Aire')
  plt.ylim(0,p_limites)
  plt.legend()
  plt.show()
  # Guardar la figura en un objeto BytesIO
  fig_buffer = io.BytesIO()
